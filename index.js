...
[Add To Your Server](https://discord.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=2117594495)
...
Web_Socket: ${Math.round(client.ping)} ms 📶\`\`\``)
  }
})
...
#unmute hello hi`))
  ...
  const Arole_client = JSON.parse(fs.readFileSync("./data/AutoRole.json", "utf8"));
client.on('message', message => {
if(message.channel.type === "dm") return;
if(message.author.bot) return;
   if(!Arole_client[message.guild.id]) Arole_client[message.guild.id] = {
    role: "member"
  }
const channel = Arole_client[message.guild.id].role
  if (message.content.startsWith(prefix + "autorole")) {
    if(!message.member.hasPermission(`MANAGE_GUILD`)) return;
    let newrole = message.content.split(' ').slice(1).join(" ")
    if(!newrole) return message.reply(`**${prefix}autorole <role name>**`)
    Arole_client[message.guild.id].role = newrole
    message.channel.send(`**${message.guild.name}'s role has been changed to ${newrole}**`);
  }
fs.writeFile("./data/AutoRole.json", JSON.stringify(Arole_client), function(e){
    if (e) throw e;
})
});
client.on("guildMemberAdd", member => {
      if(!Arole_client[member.guild.id]) Arole_client[member.guild.id] = {
    role: "member"
  }
    const sRole = Arole_client[member.guild.id].role
